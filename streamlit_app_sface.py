...
st.title("⚡ Fast Intruder Detection with MediaPipe + DeepFace (SFace)")
st.subheader("Real-Time Face Detection (MediaPipe) + Recognition (SFace)")
...
threshold = st.sidebar.slider("Cosine Similarity Threshold", 0.0, 1.0, 0.68, 0.01)
...
# Load known face embeddings using SFace
@st.cache_resource
def load_known_embeddings(directory="known_faces"):
    db = {}
    for file in os.listdir(directory):
        path = os.path.join(directory, file)
        try:
            embedding = DeepFace.represent(img_path=path, model_name="SFace", enforce_detection=False)[0]['embedding']
...
# Start detection
if run_button:
    st.write("Running MediaPipe + DeepFace (SFace) intruder detection...")
